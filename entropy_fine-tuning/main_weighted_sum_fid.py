...
import wandb
import argparse
from scorer_bi import RCGDMScorer, PenaltyScorer, Weighted_FID_Scorer
import random
import time
...
def get_grad_eval_weight(real_image, fid_images, image_eval, reward_model, device='cuda'):    
    ...
    rewards, rewards_val, fid_ = reward_model(real_image, fid_images, image_eval)
    rewards_sum = rewards.sum()
    rewards_sum.backward()  # get the gradients for each sample in the batch
    grads = image_eval.grad
    biases = -torch.einsum('bijk,bijk->b', grads, image_eval) + rewards  # r(x) - <grad, x>
    return grads, biases, rewards_val, fid_
...
def run_for_lambda(args, reward_level, lambda_scale):
    ...
    if reward_level == 'penalty':
        ...
        reward_model = RCGDMScorer().to(device)
    elif reward_level == 'weighted_sum':
    ...
    image_fids = np.zeros([args.opt_steps, args.repeat_epoch, args.bs])
    ...
    for n in range(args.repeat_epoch):
        ...
        for k in range(args.opt_steps):
            ...
            fid_images = torch.stack([
            torch.tensor(np.array(img.resize((224, 224)).convert("RGB"))).permute(2, 0, 1)
            for img in image_
            ])

            grads, biases, rewards, fid_score = get_grad_eval_weight(pretrain_images, fid_images, image_eval_, reward_model)
    ...
    image_fids_mean = np.mean(image_fids, axis=2)
    np.savetxt(out_dir + '/fids_mean_repeat.csv', image_fids_mean, delimiter=',')
    image_fids_std = np.std(image_fids_mean, axis=1)
    np.savetxt(out_dir + '/fids_std_in_repeats.csv', image_fids_std, delimiter=',')

    image_fids = image_fids.reshape(args.opt_steps, -1)
    np.savetxt(out_dir + '/fids.csv', image_fids, delimiter=',')
    mean_fids = np.mean(image_fids, axis=1)
    std_fids = np.std(image_fids, axis=1)
    np.savetxt(out_dir + '/mean_fids.csv', mean_fids, delimiter=',')
    np.savetxt(out_dir + '/std_fids.csv', std_fids, delimiter=',')
...
def main():
    ...
    for lambda_scale in lambda_values:
        run_for_lambda(args, "weighted_sum",lambda_scale)
