...
import wandb
import argparse
from scorer import RCGDMScorer, PenaltyScorer
import random
import time
...
def parse_args():
    ...
    parser.add_argument("--gemma", type=float, default=1., help="penalty of lower level gap")
    parser.add_argument("--kl_estimate", type=str, choices=['est1', 'est2'], required=True, help='Choose between method 1 and method 2 for estimating kl divergence')
    args = parser.parse_args()
    return args

def get_grad_eval(ims, reward_model, prompt=[], device='cuda'):    
    ims = ims.to(device)
    ims.requires_grad = True
    rewards = reward_model(ims, prompt)
...
def generate_from_pretrained(args):
    ...
    remark = f'seed{args.seed}totalbs{total_bs}_{args.prompt}'
    ...
    return pretrain_images


def generate_w_reward(args, reward_level, lambda_scale):
    device = args.device if torch.cuda.is_available() else "cpu"
    total_bs = args.bs * args.repeat_epoch
    remark = f'target{args.target}guidance{args.guidance}level{reward_level}seed{args.seed}totalbs{total_bs}_{args.prompt}'
    out_dir = args.out_dir + '/' + remark
    os.makedirs(out_dir, exist_ok=True)

    print("-" * 50)
    print("reward_level:", reward_level)
    ...
    if reward_level == 'penalty':
        reward_model = PenaltyScorer(args.gemma).to(device)
    ...
    for n in range(args.repeat_epoch):
        ...
        for k in range(args.opt_steps):
            ...
            if k < args.opt_steps - 1:
                image_, image_eval_= sd_model(prompt, num_images_per_prompt=args.bs, latents=init_i, **image_size_kwargs)
            else:
                image_, image_eval_, all_guidance = sd_model(prompt, num_images_per_prompt=args.bs, latents=init_i, compute_all_guidance=True, **image_size_kwargs)
            image_ = image_.images
            grads, biases, rewards = get_grad_eval(image_eval_, reward_model, [prompt]*args.bs)
    ...
    #pretrain_images/final_opt_images are tensors of prentrained/guided samples of sample size args.bs
    # to_tensor = transforms.ToTensor()
    # opt_images = [to_tensor(image) for image in opt_images_step[args.opt_steps-1][-1]]
    
    all_norm = [torch.norm(g, p=2, dim=(1, 2, 3), keepdim=True).reshape(-1) for g in all_guidance]
    all_norm = torch.stack(all_norm, dim=1)
    # all_norm: bs * time_steps
    kl_estimate = all_norm.mean(dim=1).mean()
    
    return kl_estimate
   

# def estimate_kl_w_samples(args, reward_level, lambda_scale, sample_latents):
#     device = args.device if torch.cuda.is_available() else "cpu"
#     image_size_kwargs = {"height": args.image_size, "width": args.image_size}
...
#     ### Gradient Guided Optimization
#     target = args.target
#     guidance = args.guidance / lambda_scale
#     sd_model = GradGuidedSDPipeline.from_pretrained("runwayml/stable-diffusion-v1-5", local_files_only=True)
#     sd_model.to(device)

#     torch.manual_seed(args.seed)
#     init_latents = sample_latents.to(device)

#     # opt_start_time = time.time()
#     # opt_images_step = defaultdict(list)
#     # opt_latents_step = defaultdict(list)
#     # image_rewards = np.zeros([args.opt_steps, args.repeat_epoch, args.bs])
    
#     sd_model.set_target(target)
#     sd_model.set_guidance(guidance)
    
#     all_guidance = sd_model.compute_all_steps_guidance(reward_model, prompt, num_images_per_prompt=args.bs, latents=init_latents, **image_size_kwargs)
#     all_norm = [torch.norm(g, p=2, dim=(1, 2, 3), keepdim=True).reshape(-1) for g in all_guidance]
#     all_norm = torch.stack(all_norm, dim=1)
    
#     # need to divide other t dependent constants, all_norm: bs * time_steps
#     kl_estimate = all_norm.sum(dim=1).mean()
...
def main():
    ...
    for _ in range(total_iterations):
        if args.kl_estimate == 'est1':
            device = args.device if torch.cuda.is_available() else "cpu"
            pretrain_images = generate_from_pretrained(args)
            num_imgs = len(pretrain_images)
            
            #r2
            reward_model = RCGDMScorer()
            to_tensor = transforms.ToTensor()
            imgs = torch.stack([to_tensor(img) for img in pretrain_images])
            r2_values = reward_model(imgs).squeeze()
            
            #r1
            clip_score_fn = partial(clip_score, model_name_or_path="openai/clip-vit-base-patch16")
            prompts = [args.prompt] * args.bs
            clip = [clip_score_fn(to_tensor(img), prompt).detach() for img, prompt in zip(pretrain_images, prompts)]
            r1_values = torch.stack(clip)
            
            lambda_grad = - 1/lambda_scale * r1_values.mean() - args.gemma * torch.log(torch.exp(r2_values/lambda_scale).mean()) \
                + args.gemma * torch.log(torch.exp((r1_values/args.gemma + r2_values)/lambda_scale).mean())
            

        elif args.kl_estimate == 'est2':
            kl_lower = generate_w_reward(args, "lower", lambda_scale)
            kl_penalty = generate_w_reward(args, "penalty", lambda_scale)
            lambda_grad = args.gemma * (kl_penalty - kl_lower) / 2
        
        # step to optimize lambda
        lambda_scale = max(lambda_scale - step_size * lambda_grad, 0.0)
