...
def main(args, start, end, tau_value, epsilon):
    ...
    output_file = "results_bayesian_epsilon/DDIM/tau_{:.3f}_start_{:.3f}_end_{:.3f}_s_{:.3f}/FID/FID_scores.csv".format(tau_value,start,end,epsilon)
    ...
    with open(output_file, "w", newline="") as f:
        ...
        for i in range(args.epochs):
            ...
            os.makedirs("results_bayesian_epsilon/DDIM/tau_{:.3f}_start_{:.3f}_end_{:.3f}_s_{:.3f}",exist_ok=True)
            torch.save(ckpt,"results_bayesian_epsilon/DDIM/tau_{:.3f}_start_{:.3f}_end_{:.3f}_s_{:.3f}/steps_{:0>8}.pt".format(tau_value,start,end,epsilon,global_steps))
            ...
            save_image(samples,"results_bayesian_epsilon/DDIM/tau_{:.3f}_start_{:.3f}_end_{:.3f}_s_{:.3f}/steps_{:0>8}.png".format(tau_value,start,end,epsilon,global_steps),nrow=int(math.sqrt(args.n_samples)))
...
search_space = [
    Real(0, 0.4, "uniform", name="start"),         # Example range for start
    Real(0.8, 1, "uniform", name="end"),        # Example range for end
    Real(1, 4, "uniform", name="tau_value"), # Tau value (log-uniform range)
    Real(0.005, 0.04, "uniform", name="epsilon")   # Epsilon value (example range)
]
...
def objective(**params):
    ...
    mean_fid_score = main(args, start, end, tau_value, epsilon)
    ...
    print(
        f"Start: {start:.3f}, End: {end:.3f}, Tau: {tau_value:.3f}, "
        f"Epsilon: {epsilon:.4f}, Mean FID: {mean_fid_score:.4f}, Time: {iteration_time:.2f} seconds"
    )
    
    # Store details for logging and analysis (optional)
    if isinstance(mean_fid_score, torch.Tensor):
        mean_fid_score = mean_fid_score.item()

    # Save parameter values and iteration times (optional)
    np.savetxt("scheduler_values.csv", np.array([start, end, tau_value, epsilon]), delimiter=",")
...
if __name__=="__main__":
    ...
    print(f"Best Start: {result.x[0]:.4f}")
    print(f"Best End: {result.x[1]:.4f}")
    print(f"Best Tau: {result.x[2]:.4f}")
    print(f"Best Epsilon: {result.x[3]:.4f}")
